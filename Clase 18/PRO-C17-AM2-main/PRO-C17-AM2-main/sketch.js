...
function preload(){
  ...
  estrellado = loadImage("pajarito.png")
...
if(pajaritosmax.isTouching(dinosaurio)){
 ...
 pajaromuerto = createSprite(100,150)
  pajaromuerto.scale = 0.5
  pajaromuerto.addImage("morido",estrellado)
  pajarote.add(pajaromuerto)
  }

  if(municion.isTouching(dinosaurio)){
    gameState = "gameOver"
   superFinal.visible = true
  volver.visible = true
  console.log("toca balas")
  }
...
function volar(){
  ...
  if(frameCount%500 === 0){
     pajaros = createSprite(600,150,20,20)
...
function jefeFinal(){
  ...
  piso.velocityX = -8
pajaritosmax.destroyEach()
Bowser = createSprite(580,90,50,180)
balas()
matarJefe()
...
function balas(){
  if(frameCount%50===0){
    fuego = createSprite(550,155,60,10)
    fuego.velocityX= -10 //incrementar velocidad por cada laser que pase; porpuesta agregarlo a un grupo 
municion.add(fuego)

    fuego.lifetime= 200 

  console.log("longitud del grupo" + municion.lenght)
...
function matarJefe(){
  if(frameCount%1000===0){
    laser = createSprite(500,170,20,20)
    laser.velocityX = -13
    laser.lifetime = 200
  }
}
